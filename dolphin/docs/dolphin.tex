...
在音場圖中，每個聲部的樂器(以X加上矩型框表示)會對應到舞台上的一個虛擬聲源，，如圖 \ref{fig:stereo_editor}。當虛擬聲源沿著音場的橫坐標移動便會改變虛擬聲源相對於聽者 /* 在哪裡?  */的方位，而當虛擬聲源沿著縱座標移動則會改變虛擬聲源相對於聽者的距離。  /* 如何使用? 應在CH3說明. */
...
當手機響時，我們能僅憑鈴聲找到手機；有人叫我們的名字時，我們也能立刻找到呼喚者。聽到聲音時，我們不僅試著理解聲音的內容，也會自動推測聲源的位置，許多線索可用來推測這項資訊。我們的一對耳朵分別位於頭部兩側，相距約21.5 公分，若聲源偏離我們的正前或正後方，由於聲源和左右耳的距離不一，兩耳接收到聲音的時間和感受到的音量便會不同，造成兩耳時間差(interaural time difference, ITD)和兩耳音量差(interaural level difference , ILD)。此外，聲波通過環境(如地面、牆壁)或身體(如頭部、肩膀及外耳)等物體，其頻率也會因反射或干擾而變化。我們的大腦便能根據這些資料推測聲源的位置。

\figurewithcaption
{fig:ears}
{img/ears.png}
{2.0}
{聲音由右方傳來時，聽者左右耳接收到聲音的時間、強度和頻率皆不同}

% how can we create stereo sound?
然而，大腦有時也會誤判聲源的位置。將一對喇叭置於聽者前方，兩喇叭和聽者等距，喇叭和聽者會形成一個以兩喇叭連線為底邊的等腰三角形；若兩隻喇叭以一樣的音量播放一樣的音樂，由於聲音以同樣的強度同時傳進聽者的兩耳，聽者會以為音樂是由兩喇叭中間傳來，此現象又稱為phantom center \cite{maxing_audio}{p185}。我們利用這種錯覺來製造立體聲(stereo)的效果。當左喇叭的音量比右喇叭大時，我們會以為聲源偏向左邊；而當右喇叭的音量比左喇叭大時，聲源會偏向右邊。藉由改變左右喇叭的音量，我們甚至可以模擬聲源在聽者前方移動的效果。在一九三零年代，華特迪士尼的工程師便利用這個技術製作出第一部含有立體聲的電影「幻想曲」\cite{fantasia}，此技術後來稱為panning。
...
今日，panning 可透過調整混音器上的聲像調整鈕(pan pot, panoramic potentiometer)來設定。聲像調整鈕是一個旋鈕，其設定通常以時間表示，稱為聲像鐘(pan clock)；轉至七點鐘方向，聲音會完全從左喇叭的位置傳來(稱為hard left 或extreme left)；轉至五點鐘方向會從右喇叭傳來(稱為hard right 或extreme right)，十二點鐘向聲音則在正前方(稱為hard center 或dead center)，見圖 \ref{fig:panpot}。

不過Panning 僅能模擬聲源相對於聽者的方位，我們還需要距離的資訊才能定位出聲源的位置。我們依賴音量、回聲、頻率和移動(如都卜勒效應)來輔助我們判斷聲源距離。 然而，和聲音的方位比起來，人腦對聲音距離的推測能力仍相當有限\cite{wiki_sound_localization}。

%[ design
透過喇叭或耳機聆聽立體聲的音樂時，我們會在腦海中建立一個音場(stereo image, stereo field)。音場就像是一個虛擬的舞台，各個聲部的樂器分別安放在舞台上的不同位置。

\figurewithcaption
{fig:stage}
{img/stage.png}
{1.0}
{音場的俯視圖。在上圖中，L 代表左喇叭，R 代表右喇叭，聽者則位於兩喇叭前方。若聽者移近喇叭，音場會隨之變寬；若聽者偏離中央音場也會變形}

立體聲編輯器希望以圖像來呈現音場，讓作曲者在圖上以拖拉的方式，設計各聲部在舞台上的位置。然而，喇叭的擺放位置、聽者的位置，聽者面向的方向，以及聽者所處的環境皆會影響音場的大小和形狀(見圖 \ref{fig:stage})，因此我們統一以一個矩型的音場圖來代表音場，橫座標代表樂器相對於聽者的方位，縱座標代表樂器和聽者的距離。使用矩形的另一個好處是我們可以用比扇形大的視窗區域來表示較為擁擠的舞台前方。音場圖和實際音場的對應如圖 \ref{fig:stage_mapping}所示。
...
% location
我們以Java Sound MIDI API 來實作樂器的方位和距離。樂器的方位以panning 實作。在MIDI 中，panning 可透過Pan 訊息(CC#10)來控制。0 到127 代表聲像調整鈕的七點到五點。由於0 至127 的中心是63.5，無法在MIDI 訊息中表示，故其有效範圍定為1 至127，64 為hard center，0 和1 皆視為hard left \cite{http://www.midi.org/techspecs/rp36.php}。

% distance


樂器的距離則以音量實作。由於聲音的音量和距離呈反比[21]，距離愈遠音量愈小。但由於使用者的MIDI 裝置 、作業系統的音量設定、擴大器的音量設定、喇叭、音效晶片、聽者所處的位置，和聽者所處的環境皆會影響聽者實際接收到的音量。這些限制使我們難以模擬音場中樂器和聽者的絕對距離，故我們以音場圖的最下方代表樂器和聽者最近的距離，音場圖的最上方代表無限遠。我們使用MIDI 中的Channel Volume 訊息(CC#7) 來控制樂器的音量。當音量最大時(127) 代表最近，音量最小時(0) 代表無限遠。
