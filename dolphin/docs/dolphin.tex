...
然而，大腦有時也會誤判聲源的位置。將一對喇叭置於聽者前方，兩喇叭和聽者等距，喇叭和聽者會形成一個以兩喇叭連線為底邊的等腰三角形；若兩隻喇叭以一樣的音量播放一樣的音樂，由於聲音同時以同樣的強度傳進聽者的兩耳，聽者會以為音樂是由兩喇叭中間傳來，此現象又稱為phantom center \cite{maxing_audio}{p185}。藉由改變左右喇叭的音量，我們可以模擬聲源在聽者前方移動的效果。當左喇叭的音量比右喇叭大時，我們會以為聲源偏向左邊；而當右喇叭的音量比左喇叭大時，聲源會偏向右邊。在一九三○年代，華特迪士尼的工程師便利用這個效果製作出第一部含有立體聲的電影「幻想曲」\cite{fantasia}。

%figure pan pot

% how can we generate stereo sound in MIDI?
今日，我們可調整混音器上的pan pot(panoramic potentiometer)來移動聲源的位置，此技術稱為panning。pan pot 是一個旋鈕，其位置通常以時間表示(稱為pan clock)；轉至七點鐘方向，聲音會完全從左喇叭的位置傳來(稱為hard left)；轉至五點鐘方向會從右喇叭傳來(稱為hard right)，十二點鐘向則在正前方(稱為hard center 或dead center)。在MIDI 中則是透過Pan 訊息(CC#10)來控制panning，以8 的bit(0 到127) 代表pan pot 的七點到五點。由於0 至127 的中心是63.5，無法在MIDI 訊息中表示，故其有效範圍定為1 至127，64 為hard center，0 和1 皆可用來代表hard left \cite{http://www.midi.org/techspecs/rp36.php}。
