...
今日，我們可調整混音器上的pan pot(panoramic potentiometer)來移動聲源的位置，此技術稱為panning。pan pot 是一個旋鈕，其位置通常以時間表示(稱為pan clock)；轉至七點鐘方向，聲音會完全從左喇叭的位置傳來(稱為hard left)；轉至五點鐘方向會從右喇叭傳來(稱為hard right)，十二點鐘向則在正前方(稱為hard center 或dead center)。在MIDI 中則是透過Pan 訊息(CC#10)來控制panning，以8 的bit(0 到127) 代表pan pot 的七點到五點。由於0 至127 的中心是63.5，無法在MIDI 訊息中表示，故其有效範圍定為1 至127，64 為hard center，0 和1 皆可用來代表hard left，
