...
除了產生樂譜之外，多數樂譜編輯器也可將編輯中的樂譜播放出來，功能類似音樂播放器；然而，音樂播放器主要用來播放錄製好的音訊檔，而樂譜編輯器的播放功能則用來播放編輯中的樂譜。樂譜編輯器通常也可產生音訊檔，該檔案可直接用來交流、分享，或交由數位音訊軟體做進一步的混音和後製。
...
Java Scripting API位於javax.script中，是一個獨立於腳本語言(scripting language)的直譯器使用介面。目前Java SE內建有JavaScript的直譯器，使用者也可自行下載如Python、Ruby、AWK等直譯引擎(scripting engine)\footnote{這個網頁收集了許多可供Java Scripting API 使用的直譯引擎：\url{http://java-source.net/open-source/scripting-languages}}。
...
Java Sound API中的Sequence僅記錄MIDI訊息而不是音符，是屬於低階的MIDI檔模型，因此並不適合所見即所得的樂譜編輯器。因此，我們為Dolphin定義了一套高階的模型，模型的結構如圖\ref{fig:model}所示。
...
樂譜還具有復原(undo)、重做(redo)，以及通知改動的機制，請參考\ref{sec:changes} 節。
...
如圖 \ref{fig:play}所示，Sequencer在播放Sequence時，會將Sequence內的MIDI訊號依時間戳記依序送出，音源器(Synthesizer)根據接收到的MIDI訊號自音源庫中提取對應的電子訊號並送給喇叭(speaker, loudspeaker)，喇叭則根據電子訊號產生聲音再由聽者所接收。
移到 2.1    **********/ 
...
今日，panning 可透過混音器上的聲像調整鈕(pan pot, panoramic potentiometer)來調整。聲像調整鈕是一個旋鈕，其設定通常以時間表示，稱為聲像鐘(pan clock)；轉至七點鐘方向，聲音會完全從左喇叭的位置傳來(稱為hard left 或extreme left)；轉至五點鐘方向會從右喇叭傳來(稱為hard right 或extreme right)，十二點鐘向聲音則會在正前方(稱為hard center 或dead center)，見圖 \ref{fig:panpot}。
...
要定出樂器在舞台上的位置，我們需要解決樂器相對於聽者的方位和距離兩個問題。樂器的方位可以Panning 來實作。要注意的是，各喇叭和聽者的相對位置，以及聽者面向的方向皆會影響方位，造成不同寬度和位置的音場(見圖 \ref{fig:stage})。模擬樂器的距離會遇到更多問題。以音量而言，我們不但需要知道各樂器實際能產生多少能量，還需控制使用者作業系統的音量設定、擴大器的音量設定、喇叭和聽者的距離和喇叭的擺放方式才能重製出希望的音量。以頻率而言，高頻音的衰減在近距離的音樂聆聽環境並不明顯。回聲由聽者所處的環境所決定。而各樂器通常也不像交通工具會處於持續移動的狀態。由於以上這些限制，我們難以藉由音量、頻率、回聲和移動來模擬音場中樂器和聽者的絕對距離。其實就算處於演奏現場，各樂器和我們的距離也難以僅憑聽覺定出，各樂器的比重或音量才是更明顯的元素。
...
% api
我們以Java Sound MIDI API 來實作樂器的方位和音量。樂器的方位以panning 實作。在MIDI 中，panning 可透過Pan 訊息(CC#10)來控制。0 到127 代表聲像調整鈕的七點到五點。由於0 至127 的中心是63.5，無法在MIDI 訊息中表示，故其有效範圍定為1 至127，64 為hard center，0 和1 皆視為hard left \cite{http://www.midi.org/techspecs/rp36.php}。樂器的音量則以MIDI 中的Channel Volume 訊息(CC#7) 來控制。當音量最大時其值為127，音量最小時其值為0。
