...
多個樂譜介面可連接至同一個樂譜，然而樂譜並不直接連結至樂譜介面，而是透過樂譜改動傾聽者(ScoreChangeListener)的scoreChanged()方法來與樂譜介面溝通，如圖 7所示。此設計基於觀察者模式(observer pattern)\cite{designPattern}，樂譜並不依賴樂譜介面，可以獨立存在。
/****
漏講其它各種view
****/
