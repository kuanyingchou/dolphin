...
當手機響時，我們能僅憑鈴聲找到手機的位置。許多線索可用來推測此資訊。由於我們的一對耳朵分別位於頭部的兩側，相距約21.5 公分，若聲源偏離我們的正前或正後方，兩耳接收到聲音的時間和感受到的聲音的強度便會不同，造成兩耳時間差(interaural time difference, ITD)和兩耳音量差(interaural level difference , ILD)。此外，聲波通過環境(如地面、牆壁)或身體(如頭部、肩膀及外耳)等物體時也會因反射造成音量及音色的變化。我們的大腦便能根據這些資訊決定聲源的位置。
...
% how did you implement stereo sound?
