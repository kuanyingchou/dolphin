...
鋼琴捲譜(piano roll notation)是另一種樂譜編輯器常缺乏的記譜法。鋼琴捲譜屬於圖像譜的一種，因形似鋼琴捲(piano roll)而得名。鋼琴捲是一種早期的音樂儲存媒介，藉由在紙捲上打孔來記錄音符的資訊，由自奏鋼琴(player piano)讀取並將音樂彈奏出來。鋼琴捲在收音機和唱片普及前的年代曾相當流行\cite{thePiano}。鋼琴捲譜以橫軸代表時間，縱軸代表音高；這樣的表示方式相當直覺，初學者也易於上手。
